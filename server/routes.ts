...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  */
  ...
  // 🔄 Route temporaire: Migration des catégories de pièces détachées
  app.post("/api/admin/migrate-spare-parts-categories", async (req, res) => {
    try {
      console.log("🔄 Démarrage migration des catégories de pièces...");

      // Mapping ancien → nouveau
      const categoryMapping: Record<string, string> = {
        "piece-voiture": "piece-voiture-utilitaire",
        "piece-moto": "piece-moto-scooter",
        // autre-piece reste inchangé
      };

      const results = {
        updated: [] as Array<{ id: number; oldCategory: string; newCategory: string }>,
        alreadyUpdated: 0,
        errors: [] as Array<{ id: number; error: string }>,
      };

      // Récupérer toutes les annonces de pièces avec anciennes catégories
      const { data: vehicles, error } = await supabaseServer
        .from("annonces")
        .select("*")
        .in("category", ["piece-voiture", "piece-moto"]);
      ...
      if (!vehicles || vehicles.length === 0) {
        ...
        return res.json({ 
          message: "Aucune annonce à migrer", 
          results: { updated: [], alreadyUpdated: 0, errors: [] }
        });
      }

      console.log(`📊 ${vehicles.length} annonces à migrer`);

      // Migrer chaque annonce
      for (const vehicle of vehicles) {
        const oldCategory = vehicle.category;
        const newCategory = categoryMapping[oldCategory];

        if (!newCategory) {
          console.log(`⏭️ Catégorie ${oldCategory} non concernée par la migration`);
        ...
        if (updateError) {
          console.error(`❌ Erreur pour annonce ${vehicle.id}:`, updateError);
          results.errors.push({ id: vehicle.id, error: updateError.message });
        } else {
          console.log(`✅ Annonce ${vehicle.id}: ${oldCategory} → ${newCategory}`);
      ...
      res.json({
        success: true,
        message: `Migration terminée: ${results.updated.length} annonces mises à jour`,
        results,
      });
    } catch (error) {
